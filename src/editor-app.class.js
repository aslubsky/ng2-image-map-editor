...
export var CursorPositionInfo = (function () {
...
export var EditorApp = (function () {
    ...
    EditorApp.prototype.onSvgClick = function (e) {
        if (this.state.appMode === 'drawing' && !this.state.isDraw && this.state.currentType) {
            this.setIsDraw(true);
            this.state.newArea = Area.CONSTRUCTORS[this.state.currentType].createAndStartDrawing(Utils.getRightCoords(e.pageX, e.pageY));
        }
    };
        ...
        switch (e.keyCode) {
            ...
            case EditorApp.KEYS.C:
                if (this.state.appMode === 'editing' && this.state.selectedArea && ctrlDown) {
                    var Constructor = Area.CONSTRUCTORS[area_params.type], area_params = this.state.selectedArea.toJSON();
                    if (Constructor) {
